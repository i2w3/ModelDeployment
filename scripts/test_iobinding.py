...
if __name__ == "__main__":
    ...
    # IO binding for better performance
    binding = session.io_binding()
    output_tensor_ort = ort.OrtValue.ortvalue_from_shape_and_type(output_shape, np.float32, 'cuda', 0)
    binding.bind_output(
        name=output_name,
        device_type='cuda',
        device_id=0,
        ...
        shape=output_shape,
        buffer_ptr=output_tensor_ort.data_ptr()
    )
    ...
    for _ in range(runs):
        ...
        input_tensor_ort = ort.OrtValue.ortvalue_from_numpy(det_input.blob, 'cuda', 0)
        binding.bind_input(
            name=input_name,
            device_type='cuda',
            device_id=0,
            element_type=np.float32,
            shape=det_input.blob.shape,
            buffer_ptr=input_tensor_ort.data_ptr()
        )
        
        session.run_with_iobinding(binding)
        outputs = [output_tensor_ort.numpy()]
